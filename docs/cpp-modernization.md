...
> **Status**: More structured (omemo session, message, etc.); ongoing (as of 2026-06-03)
...
- Continued ... ( + omemo session_flow MUC ch_it, ch, buf; message last emplace; added maybe_unused to omemo unused funcs for build).
...
| Structured bindings in for/find | few | more (... + omemo session ch_it/MUC, message emplace, etc.) |
...
- **`std::expected`** can be adopted further in error-returning functions — the
  pattern is established, adoption should be incremental (parse_* , load_tofu_trust, og_cache_lookup, aes_mode, avatar_load, axolotl_omemo_*, 2x mam_cache_lookup done; more in
