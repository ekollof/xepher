...
normalize_version() {
    ...
    if ! printf '%s' "$ver" | grep -E '^[0-9]+\.[0-9]+\.[0-9]+([.-][0-9A-Za-z.+~]+)?$' >/dev/null; then
...
check_packaging_versions() {
    ...
    arch_ver=$(packaging_version_from "${root}/packaging/arch/PKGBUILD" 's/^pkgver=//p')
    rpm_ver=$(packaging_version_from "${root}/packaging/rpm/weechat-xmpp.spec" 's/^Version:[[:space:]]*//p')
    deb_ver=$(packaging_version_from "${root}/packaging/debian/changelog" 's/^xepher (//p' | sed 's/-.*//')
    fbsd_ver=$(packaging_version_from "${root}/packaging/freebsd/Makefile" 's/^PORTVERSION=//p')
    obsd_ver=$(packaging_version_from "${root}/packaging/openbsd/Makefile" 's/^V =[[:space:]]*//p')
...
check_git_state() {
    ...
    if [ "$local_sha" != "$remote_sha" ]; then
        die "local $DEFAULT_BRANCH ($(short_sha "$local_sha")) differs from origin/$DEFAULT_BRANCH ($(short_sha "$remote_sha")); push or pull first"
    fi

    log "git state OK ($DEFAULT_BRANCH @ $(short_sha "$local_sha"), clean)"
}
    ...
    if [ "$DRAFT" -eq 1 ]; then
        if [ "$GENERATE_NOTES" -eq 1 ]; then
            if [ -n "$RELEASE_NOTES" ]; then
                run gh release create "$tag" --repo "$REPO" --title "$RELEASE_TITLE" \
                    --verify-tag --draft --generate-notes --notes "$RELEASE_NOTES"
            else
                run gh release create "$tag" --repo "$REPO" --title "$RELEASE_TITLE" \
            ...
            run gh release create "$tag" --repo "$REPO" --title "$RELEASE_TITLE" \
                --verify-tag --draft --notes "$RELEASE_NOTES"
        else
            run gh release create "$tag" --repo "$REPO" --title "$RELEASE_TITLE" \
                --verify-tag --draft --notes-from-tag
        fi
    else
        if [ "$GENERATE_NOTES" -eq 1 ]; then
            if [ -n "$RELEASE_NOTES" ]; then
            ...
            run gh release create "$tag" --repo "$REPO" --title "$RELEASE_TITLE" \
                --verify-tag --notes "$RELEASE_NOTES"
        else
            run gh release create "$tag" --repo "$REPO" --title "$RELEASE_TITLE" \
