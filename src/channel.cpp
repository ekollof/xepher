                                                                       ...
                                                                       add_member_opts opts)
{
    ...
    {
        weechat::UiPort::for_buffer(buffer)->printf_date_tags_network(0, "log2",
            fmt::format("MUC: {}", id));
        return std::nullopt;
    }
    ...
    // XEP-0384 §5.8.1: only member/admin/owner real JIDs are OMEMO recipients.
    if (real_jid && type == weechat::channel::chat_type::MUC && muc_supports_omemo())
    {
        const bool eligible = !known_user
            || !known_user->profile.affiliation
            || is_omemo_recipient_affiliation(*known_user->profile.affiliation);
        if (eligible)
...
void weechat::channel::notify_omemo_missing_real_jids(weechat::UiPort &ui) const
{
    ...
    ui.printf_date_tags_error(0, "notify_none",
...
bool weechat::channel::is_omemo_recipient_affiliation(std::string_view affiliation)
{
    return affiliation == "member" || affiliation == "admin" || affiliation == "owner";
}
...
void weechat::channel::register_omemo_recipient(std::string_view bare_jid)
{
    ...
    // XEP-0384 §5.8.2: prefetch devicelists for discovered recipients.
    account.omemo.note_peer_traffic(account.context, normalized);
...
void weechat::channel::unregister_omemo_recipient(std::string_view bare_jid)
{
    ...
    if (omemo_recipient_jids.erase(normalized) == 0)
        return;

    const std::string prefix = fmt::format("{}/", normalized);
    std::erase_if(omemo.pending_muc_bundle_keys,
...
void weechat::channel::maybe_disable_muc_omemo()
{
    ...
    weechat::UiPort::for_buffer(buffer)->printf_date_tags_network(0, "notify_none",
        fmt::format("OMEMO disabled: room is {} (XEP-0384 requires non-anonymous)", reason));
}
                                     ...
                                     std::string_view alt)
{
    ...
    {
        weechat::UiPort::for_buffer(buffer)->printf_date_tags_error(0, "notify_none",
            fmt::format("image too large for XEP-0231 BoB (max {} bytes)", ::xmpp::k_bob_max_payload_bytes));
        return WEECHAT_RC_ERROR;
    }
    ...
    {
        weechat::UiPort::for_buffer(buffer)->printf_date_tags_error(0, "notify_none",
            "BoB send requires plaintext; disable OMEMO or use /upload");
        return WEECHAT_RC_ERROR;
    }
...
int weechat::channel::send_message(std::string_view to, std::string_view body, bool skip_probe)
{
    ...
    if (account.omemo && omemo.enabled && type == weechat::channel::chat_type::MUC)
    {
        ...
        {
            weechat::UiPort::for_buffer(buffer)->printf_date_tags_error(0, "notify_none",
                "OMEMO group chat requires a non-anonymous room (XEP-0384 §5.8)");
            return WEECHAT_RC_ERROR;
        }
        if (omemo_recipient_jids.empty())
        {
            weechat::UiPort::for_buffer(buffer)->printf_date_tags_error(0, "notify_none",
                "OMEMO not ready yet (waiting for member/admin/owner lists)");
            return WEECHAT_RC_ERROR;
        }
        if (!all_occupants_have_real_jid())
        {
        ...
        {
            weechat::UiPort::for_buffer(buffer)->printf_date_tags_error(0, "notify_none",
                fmt::format("OMEMO: waiting for {} occupant bundle fetch(es) (XEP-0384 §5.8.2)",
    ...
    else if (pgp.enabled && !pgp.ids.empty())
    {
        ...
        {
            weechat::UiPort::for_buffer(buffer)->printf_date_tags_error(0, "notify_none", "PGP Error");
...
void weechat::channel::fetch_mam(const char *id, time_t *start, time_t *end, const char* after)
{
    ...
    if (!after && buffer)
    {
        ...
        weechat::UiPort::for_buffer(buffer)->printf_date_tags_network(0, "xmpp_mam_fetch,notify_none,no_log",
            fmt::format("Fetching history: {} → {}", start_str, end_str));
    }
