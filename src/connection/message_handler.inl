...
message_handler_after_omemo:
    ...
    // (The actual content was already rendered by reactions/retract code.)
    // NOTE: <reply> (XEP-0461) is intentionally excluded here — the reply
    // body IS the message text; only the "↪ excerpt" prefix is generated by
    // the reply handler, and it is prepended to the body, not a replacement.
    ...
    {
        // Suppress body only for stanza types whose handler already renders
        // the full content: reactions, retract, apply-to (moderation).
        // Reply is NOT in this list — the body is the reply text itself.
