...
void command__init()
{
    ...
    hook = weechat_hook_command(
        ...
        N_("       check: query server for published OMEMO devicelist and bundle\n"
           ...
           "  /omemo reset-keys        : wipe entire OMEMO key database (own keys too)\n"
           "  /omemo prune             : remove peer sessions/bundles/device lists; keep own keys\n"
    ...
    hook = weechat_hook_command(
        ...
        N_("flags: m=moderated i=members-only k=password p=hidden "
           ...
           "in shell history.\n\n"
           "Apply is done as a single submit on --confirm: the plugin sends "
           "a muc#owner IQ with the changed fields plus FORM_TYPE. If a "
           "previously-cached form exists (from a join, /create --reserved, "
           "or a background GET) the submit includes all original fields so "
           "server-side values are preserved. Otherwise a partial submit is "
           "sent — most servers accept this; if yours does not, you can "
           "trigger a full GET by running /setmodes with --confirm once on "
           "an empty flag spec to populate the cache, then re-run.\n\n"
           "Examples:\n"
           "  /setmodes +m\n"
