...
- [ ] Complete 0184 builder (request) + migrate channel sends (regular + file).
- [ ] Add xep-0334.inl (hints) + migrate stores/no-*.
- [ ] Migrate file-share compat (fallback/SIMS/OOB) + main envelope to fluent (use 0428/0447/0359 etc mixins); deprecate make_child.
- [ ] Migrate account.cpp feeds/pubsub to stanza::iq + xep0060; share SFS builder.
- [ ] Unify version/time replies (connection.cpp) to stanza::iq or document.
- [ ] Extend 0447 + worker for hash agility + purpose (0363); re-fetch 0363/044x/0300 + commit specs.
...
- [ ] Fill gaps in caps/MAM/carbons/SM if emission or critical parse deviations.
- [ ] Inbound improvements (lenient where C-ABI allows).
- [ ] More <file> attrs (date etc).
